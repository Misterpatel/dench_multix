$(document).ready(function () {
    "use strict";
    var pageForm = $("#blog_category_form");
    if (pageForm.length) {
    ...
    $("#blog_category_submit_btn").on("click", function (e) {
        e.preventDefault();
        if (!pageForm.valid()) {
            return false;
        }
        ...
        $.ajax({
            url: httpPath + "manage-blog-category",
            headers: {
            ...
            success: function (response) {
                ...
                if (obj.res == "1") {
                    ...
                    setInterval(function () {
                        window.location = httpPath + "blog-category";
                    }, 1000);
                } else if (obj.res == "3") {
                    $("#blog_category_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
...
$(document).ready(function () {
    ...
    var fileDataTable = $('#file-datatable').DataTable({
        ...
        iDisplayLength: 10,
        sAjaxSource: httpPath + "fetch-blog-category",
...
function delete_record(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: httpPath + "delete-blog-category",
                headers: {
