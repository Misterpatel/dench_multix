$(document).ready(function () {
    ...
    $("#testimonial_submit_btn").on("click", function (e) {
        e.preventDefault();
        if (!pageForm.valid()) {
            return false;
        }
        ...
        $.ajax({
            url: httpPath + "manage-testimonial",
            headers: {
            ...
            success: function (response) {
                ...
                if (obj.res == "1") {
                    ...
                    setInterval(function () {
                        window.location = httpPath + "testimonial";
                    }, 1000);
                } else if (obj.res == "3") {
                    $("#testimonial_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
...
$(document).ready(function () {
    ...
    var fileDataTable = $('#file-datatable').DataTable({
        ...
        iDisplayLength: 10,
        sAjaxSource: httpPath + "fetch-testimonial",
...
function delete_record(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: httpPath + "delete-testimonial",
                headers: {
