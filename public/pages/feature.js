$(document).ready(function () {
    ...
    $("#feature_submit_btn").on("click", function (e) {
        e.preventDefault();
        if (!pageForm.valid()) {
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (obj.res == "1") {
                    $("#feature_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
                    toastr.success("Data Added Successfully.");
                    setInterval(function () {
                        window.location = httpPath + "feature";
                    }, 1000);
                } else if (obj.res == "3") {
                    $("#feature_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
...
$(document).ready(function () {
    ...
    var fileDataTable = $('#file-datatable').DataTable({
        ...
        iDisplayLength: 10,
        sAjaxSource: httpPath + "fetch-feature",
