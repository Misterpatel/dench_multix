$(document).ready(function () {
    ...
    $("#team_member_submit_btn").on("click", function (e) {
        e.preventDefault();
        if (!pageForm.valid()) {
            return false;
        }
        ...
        $.ajax({
            url: httpPath + "manage-team-member",
            headers: {
            ...
            success: function (response) {
                ...
                if (obj.res == "1") {
                    $("#team_member_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
                    toastr.success("Data Added Successfully.");
                    setInterval(function () {
                        window.location = httpPath + "team-member";
                    }, 1000);
                } else if (obj.res == "3") {
                    $("#team_member_form").trigger("reset");
                    $("#file-datatable").DataTable().ajax.reload();
...
$(document).ready(function () {
    ...
    var fileDataTable = $('#file-datatable').DataTable({
        ...
        iDisplayLength: 10,
        sAjaxSource: httpPath + "fetch-team-member",
...
function delete_record(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: httpPath + "delete-team-member",
                headers: {
